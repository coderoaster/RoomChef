...
//                                Toast.makeText(getApplicationContext(), "가입되지 않은 계정입니다. 다시 로그인해 주세요.", Toast.LENGTH_SHORT).show();
                        ...
                        urlAddr="";
                        urlAddr = "http://" + centIP + ":8080/test/loginInsert.jsp?";
                        urlAddr = urlAddr + "email=" + result.getKakaoAccount().getEmail();
                        connectInsertData();
        ...
        @Override
        public void onSessionOpenFailed(KakaoException exception) {//세션을 여는 도중 오류가 발생한 경우 -> Toast 메세지를 띄움.
//            Toast.makeText(getApplicationContext(), "로그인 도중 오류가 발생했습니다. 인터넷 연결을 확인해주세요: "+exception.toString(), Toast.LENGTH_SHORT).show();
    ...
    private void connectInsertData(){
        try {
            InsNetworkTask insNetworkTask = new InsNetworkTask(LoginActivity.this,urlAddr);
