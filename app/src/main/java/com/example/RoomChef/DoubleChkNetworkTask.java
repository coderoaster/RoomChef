...
public class DoubleChkNetworkTask extends AsyncTask<Integer, String , Object> {
    Context context;
    String mAddr;
    ProgressDialog progressDialog;
    String count; // 이메일 카운트

    public DoubleChkNetworkTask(Context context, String mAddr) {
    ...
    protected Object doInBackground(Integer... integers) {
        ...
        return count; // 이메일 존재하면 1 아니면 0
    }
    private void Parser(String s){
        try {
            JSONObject jsonObject = new JSONObject(s);
            count = jsonObject.getString("count");
            Log.v("email count",count);
        }catch (Exception e){
            e.printStackTrace();
        }
    }
